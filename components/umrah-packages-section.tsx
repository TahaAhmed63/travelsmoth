  ...
  return (
    <section className="py-20 bg-gradient-to-br from-cream-50 via-white to-gold-50">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-bold text-bronze-900 mb-4">Umrah Packages</h2>
        ...
        <div className="flex justify-center mb-12">
          <div className="bg-white rounded-2xl p-2 shadow-xl border border-bronze-100">
                  ...
                  <div className="relative h-96 lg:h-auto overflow-hidden">
                    ...
                    <div className="absolute bottom-6 left-6 text-white">
                      ...
                      {currentPackage.originalPrice && (
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-lg line-through text-gray-300">${currentPackage.originalPrice}</span>
                    ...
                    {getCurrentPackages().length > 1 && (
                      <>
                        <button
                          onClick={prevPackage}
                          className="absolute left-4 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white text-bronze-900 p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-110"
                        >
                          <ArrowLeft className="w-5 h-5" />
                        </button>
                        <button
                          onClick={nextPackage}
                          className="absolute right-4 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white text-bronze-900 p-3 rounded-full shadow-lg transition-all duration-300 hover:scale-110"
                        >
                  ...
                  <div className="p-8 lg:p-12">
                    <div className="mb-6">
                      <div className="flex items-center gap-3 mb-3">
                        <Badge
                          className={`${config.bgColor} ${config.textColor} ${config.borderColor} border font-semibold px-3 py-1`}
                        >
                      ...
                      <h3 className="text-3xl font-bold text-bronze-900 mb-2">{currentPackage.name}</h3>
                      <p className="text-lg text-bronze-600 mb-4">{currentPackage.subtitle}</p>

                      <div className="flex items-center gap-6 text-bronze-600 mb-6">
                    ...
                    {/* Hotels Section */}
                    <div className="mb-6">
                      <h4 className="text-lg font-bold text-bronze-900 mb-4 flex items-center gap-2">
                        <Hotel className="w-5 h-5 text-gold-600" />
                        Accommodations
                      </h4>
                      <div className="space-y-4">
                        {currentPackage.hotels.map((hotel: any, i: number) => (
                          <div key={i} className="border border-bronze-200 rounded-lg p-4 bg-bronze-50">
                            <div className="flex items-center justify-between mb-2">
                              <div>
                                <span className="font-semibold text-bronze-900">
                                  {hotel.city} Hotel: ({hotel.nights} Nights)
                                </span>
                              </div>
                              <div className="flex items-center gap-1">
                                {[...Array(hotel.stars)].map((_, starIndex) => (
                                  <Star key={starIndex} className="w-4 h-4 fill-gold-400 text-gold-400" />
                                ))}
                              </div>
                            </div>
                            <h5 className="text-lg font-bold text-green-700 mb-1">{hotel.name}</h5>
                            <p className="text-sm text-bronze-600">{hotel.distance}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {currentPackage.features.slice(0, 4).map((feature: string, i: number) => (
                          <div key={i} className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-bronze-700">{feature}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="flex gap-4">
                      <Button
                        variant="outline"
                        className="flex-1 border-2 border-bronze-300 text-bronze-700 hover:bg-bronze-50 py-3 text-lg font-semibold"
                      >
                      ...
                      <Button
                        ...
                        className={`flex-1 bg-gradient-to-r ${config.color} hover:opacity-90 text-white py-3 text-lg font-semibold shadow-lg`}
                      >
                        Enquire Now
                      </Button>
                    </div>
        ...
        >
          <Link href="/packages">
            <Button
              size="lg"
              className="bg-gradient-to-r from-gold-500 to-bronze-600 hover:from-gold-600 hover:to-bronze-700 text-white px-8 py-4 text-lg shadow-xl"
            >
